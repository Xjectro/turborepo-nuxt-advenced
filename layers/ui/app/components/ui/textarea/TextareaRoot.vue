<template>
  <div class="relative w-full">
    <textarea
      ...
      :type="props.type"
      class="flex items-center justify-start w-full cursor-pointer focus:cursor-text outline-none p-3 rounded-xl font-inter border border-solid placeholder:text-zinc-400 border-zinc-200 dark:border-zinc-900 bg-zinc-50 hover:bg-zinc-100 focus:bg-zinc-100 dark:bg-zinc-900/30 dark:hover:bg-zinc-900/50 dark:focus:bg-zinc-900/50 disabled:text-red-500 text-zinc-900 dark:text-white"
      @input="handleInput"
    />
    <div
      class="absolute bottom-0 right-0 m-4 pointer-events-none font-inter font-light text-black dark:text-white"
    >
      {{ modelValue?.length }}
    </div>
  </div>
